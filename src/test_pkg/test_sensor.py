...
import rospy
import cv2
import test_pkg.sensor as sen  # sensor.py 모듈을 import
import numpy as np

def main():
    rospy.init_node('sensor_node', anonymous=True)  # ROS 노드 초기화
    ...
    while not rospy.is_shutdown():
        ...
        # LiDAR 데이터 접근
        if sensor_instance.lidar_range is not None:
            # LiDAR 데이터 처리 (필요에 따라 수정)
            rospy.loginfo("LiDAR 범위 데이터: {}".format(sensor_instance.lidar_range))
