...
class XycarController:
    def __init__(self):
        self.mode = LANE_DRIVE_SECOND #AR Driving
        self.mode_msg = reset()
    ...
    def lane_callback(self, data):
        ...
        elif self.mode == LANE_DRIVE_FIRST and self.flag_obstacle != None and rospy.get_time() - self.flag_obstacle < 23.0:
            self.drive(data.angle, data.speed)
        elif self.mode == LANE_DRIVE_SECOND and self.flag_init != None and rospy.get_time() - self.flag_init < 6.0:
            self.drive(data.angle, data.speed)
        elif self.mode == LANE_DRIVE_SECOND and self.flag_rubber != None and rospy.get_time() - self.flag_rubber < 7.0:    
            self.drive(data.angle, data.speed)
